...
void setup()
{
  ...
  if(!SD.exists("TULT.csv")) {
    //membuat file
    myFile = SD.open("TULT.csv", FILE_WRITE);
    if(myFile) {
...
void loop(void){    
  ...
  if(millis()-samplingTime > samplingInterval)
  {
      ...
      //Persamaan Kalibrasi
      phVoltageCalib = (-0.4028*phValueSensor)+5.0157; //Kalibrasi tahap pertama
  ...
  if(millis()-timepoint>1000U)  //time interval: 1s
  {
    ...
    //Persamaan Kalibrasi
    ecVoltageCalib = (0.2526*ecValueSensor)+0.0015; //Kalibrasi tahap pertama
    ecConversion = (ecVoltageCalib/5.0)*20.0; //Konversi tegangan ke bentuk EC
    ecValue = (1.0298*ecConversion)+0.0172; //Kalibrasi tahap kedua
  }
  ...
  //Persamaan Kalibrasi
  tdsVoltageCalib = (0.0048*tdsValueSensor)+0.1799; //Kalibrasi tahap pertama
  tdsConversion = (tdsVoltageCalib/1000)*5.0; //Konversi tegangan ke bentuk ppm
  tdsValue = (0.9967*tdsConversion)+2.4109; //Kalibrasi tahap kedua
